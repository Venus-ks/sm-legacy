...
function get_manuscript($num=null){
	$arr[] = array("cvalue"=>1, "ctext"=>"연구논문(Research Paper)");
	$arr[] = array("cvalue"=>2, "ctext"=>"기술보고(Technical Report)");
	$arr[] = array("cvalue"=>3, "ctext"=>"해설(Review Paper)");
	$arr[] = array("cvalue"=>4, "ctext"=>"총설(General Remarks)");

	if($num){
		$temp_num = $num - 1;
		return $arr[$temp_num]['ctext'];
	}else{
		return $arr;
	}
}
...
function get_category($num=null){
	$arr[] = array("cvalue"=>1, "ctext"=>"응용지질(Applied Geochemistry & Environmental Geology)");
	$arr[] = array("cvalue"=>2, "ctext"=>"물리탐사(Geophysical Prospection)");
	$arr[] = array("cvalue"=>3, "ctext"=>"암반공학(Rock Mechanics & Rock Engineering)");
	$arr[] = array("cvalue"=>4, "ctext"=>"광물자원개발(Mineral Development)");
	$arr[] = array("cvalue"=>5, "ctext"=>"신재생에너지(New Renewable Energy)");
	$arr[] = array("cvalue"=>6, "ctext"=>"자원활용소재(Energy Resource processing)");
	$arr[] = array("cvalue"=>7, "ctext"=>"석유가스개발(Petroleum & Gas Engineering)");
	$arr[] = array("cvalue"=>8, "ctext"=>"자원경제정책(Energy and Engineering Economics)");
...
function get_result($num=null){
	$arr[] = array("cvalue"=>1, "ctext"=>"수정없이 게재가(Publication Permission)");
	$arr[] = array("cvalue"=>2, "ctext"=>"수정 후 게재가(Accepted after revision)");
	$arr[] = array("cvalue"=>3, "ctext"=>"수정 후 재심사(Re-review after revision)");
...
function get_review_type($mb_id, $status){
	global $g4;
	$sql = "select count(seq) as cnt from ad_paper where ((review_a_user = '{$mb_id}' AND review_a_result = '3') or (review_b_user = '{$mb_id}' AND review_b_result = '3') or (review_c_user = '{$mb_id}' AND review_c_result = '3') or (review_d_user = '{$mb_id}' AND review_d_result = '3') or (review_e_user = '{$mb_id}' AND review_e_result = '3') )";
	...
	*/
	...
	if($member['mb_level'] >= 6){ 
		...
		switch($data['step']){
			...
			case "14": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status31.png' border='0'>";} break; 
		...
		switch($data['step']){
			...
			case "50": $imgs = "<img src='../images/status10.png' border='0'>"; break; 
