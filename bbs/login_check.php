...
$mb_login_mode = $_POST[login_mode];
...
// 회원 레벨에 맞게 로그인 하는가? 
if(!($mb_login_mode==1 && $mb['mb_level']>=2) && !($mb_login_mode==2 && $mb['mb_level']>=4) && !($mb_login_mode==3 && $mb['mb_level']==10)) {
	alert("회원 구분에 맞게 로그인해주시기 바랍니다.\\n\\n");
}

// 차단된 아이디인가?
if ($mb[mb_intercept_date] && $mb[mb_intercept_date] <= date("Ymd", $g4[server_time])) {
    $date = preg_replace("/([0-9]{4})([0-9]{2})([0-9]{2})/", "\\1년 \\2월 \\3일", $mb[mb_intercept_date]); 
    alert("회원님의 아이디는 로그인이 제한되어 있습니다.\\n\\n처리일 : $date");
}
...
set_session('ss_mb_mode', $mb_login_mode);
