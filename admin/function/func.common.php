...
function url_auto_link($str)
{
    ...
    if (strtoupper($g4['charset']) == 'UTF-8') {
        $str = preg_replace("/([^(HREF=\"?'?)|(SRC=\"?'?)]|\(|^)((http|https|ftp|telnet|news|mms):\/\/[a-zA-Z0-9\.-]+\.[가-?\xA1-\xFEa-zA-Z0-9\.:&#=_\?\/~\+%@;\-\|\,\(\)]+)/i", "\\1<A HREF=\"\\2\" TARGET='$config[cf_link_target]'>\\2</A>", $str);
    } else {
        $str = preg_replace("/([^(HREF=\"?'?)|(SRC=\"?'?)]|\(|^)((http|https|ftp|telnet|news|mms):\/\/[a-zA-Z0-9\.-]+\.[\xA1-\xFEa-zA-Z0-9\.:&#=_\?\/~\+%@;\-\|\,\(\)]+)/i", "\\1<A HREF=\"\\2\" TARGET='$config[cf_link_target]'>\\2</A>", $str);
    }
    // 이메일 정규표현식 수정 061004
...
### Review Category
function get_category($num=null){
...
function get_result($num=null){
	foreach($GLOBALS['rule']['get_result'] as $k=>$v) {
		if($v) $arr[] = array("cvalue"=>$k+1, "ctext"=>$v);
	}
...
function get_result_2nd($num=null){
	foreach($GLOBALS['rule']['get_result_2nd'] as $k=>$v) {
		if($v) $arr[] = array("cvalue"=>$k+1, "ctext"=>$v);
	}
...
function score_result($sum){
	$score_avg = round($sum/39,2);
	if($score_avg>=4) $score_result = 1;
	else if($score_avg>=3) $score_result = 2;
	else if($score_avg>=2.7) $score_result = 3;
	else $score_result = 4;
	return $score_result;
}
...
function get_result_temp($num=null){
...
function get_review_name($mb_id){
	global $g4;
	if($mb_id){
...
function get_journal_list(){
	###
	global $g4;
	$sql = "select * from ad_journal order by seq desc";
	$res	= sql_query($sql);
...
function get_status($seq, $type=null){
	global $g4, $member;
	$sql = "select * from ad_paper where seq = '{$seq}'";
	$data	= sql_fetch($sql);
	###
	$sql = "select * from ad_paper_review where parent_seq = '{$seq}' and mb_id='{$member['mb_id']}' order by regdate desc limit 3";
	$ress	= sql_query($sql);
	while ($row = sql_fetch_array($ress)) $review[] = $row;
	###
	$sql = "select * from ad_paper_total where parent_seq = '{$seq}' order by regdate desc limit 1";
	$ress	= sql_query($sql);
	...
	if(count($total)){
		...
		$imgs = "<img src=\"../images/state_09.png\" alt=\"총평완료\"/>";

	}else if($data['settle_date']!='0000-00-00'){

		$imgs = "<img src=\"../images/state_08.png\" alt=\"총평대기\"/>";

	}else if(count($review) > 2){

		$imgs = "<img src=\"../images/state_06.png\" alt=\"편집위원장 결재대기\"/>";

	}else if(count($review) < 3 and count($review) > 0){

		$imgs = "<img src=\"../images/state_03.png\" alt=\"심사대기\"/>";

	}else if($data['review_a_conf']=="Y" || $data['review_b_conf']=="Y" || $data['review_c_conf']=="Y" || $data['review_d_conf']=="Y" || $data['review_e_conf']=="Y"){

		$imgs = "<img src=\"../images/state_02.png\" alt=\"심사중\"/>";

	}else{

		$imgs = "<img src=\"../images/state_01.png\" alt=\"접수완료\"/>";
	...
	*/
	...
	/*
	switch($data['step']){
		case "1": $imgs = "<img src=\"../images/state_01.png\" alt=\"접수완료\"/>"; break;
		case "2": $imgs = "<img src=\"../images/state_02.png\" alt=\"심사중\"/>"; break;
		case "3": $imgs = "<img src=\"../images/state_03.png\" alt=\"심사대기\"/>"; break;
		case "4": $imgs = "<img src=\"../images/state_04.png\" alt=\"논문수정\"/>"; break;
		case "5": $imgs = "<img src=\"../images/state_05.png\" alt=\"투고완료\"/>"; break;
		case "6": $imgs = "<img src=\"../images/state_06.png\" alt=\"편집위원장 결재대기\"/>"; break;
		case "7": $imgs = "<img src=\"../images/state_07.png\" alt=\"편집위원장 결재완료\"/>"; break;
		case "8": $imgs = "<img src=\"../images/state_08.png\" alt=\"총평대기\"/>"; break;
		case "9": $imgs = "<img src=\"../images/state_09.png\" alt=\"총평완료\"/>"; break;
		case "10": $imgs = "<img src=\"../images/state_10.png\" alt=\"게재승인\"/>"; break;
	}

	if($type=='review'){
		if($data['step'] > 49) $data['step'] = 5;
	}
	if($type=='writer'){
		if($data['step'] > 49) $data['step'] = 5;
	}
	*/
	...
	if($member['mb_level'] >= 6){
		switch($data['step']){
			case "1": $imgs = "<img src='../images/status14.png' border='0'>"; break;
			case "2": $imgs = "<img src='../images/status15.png' border='0'>"; break;
			case "3": $imgs = "<img src='../images/status16.png' border='0'>"; break;
			case "4": $imgs = "<img src='../images/status04.png' border='0'>"; break;
			case "5": $imgs = "<img src='../images/status18.png' border='0'>"; break;

			case "10": $imgs = "<img src='../images/status05.png' border='0'>"; break;
			case "11": $imgs = "<img src='../images/status20.png' border='0'>"; break;
			case "13": $imgs = "<img src='../images/status43.png' border='0'>"; break;
			case "14": $imgs = "<img src='../images/status07.png' border='0'>"; break;
			case "15": $imgs = "<img src='../images/status22.png' border='0'>"; break;

			case "20": $imgs = "<img src='../images/status08.png' border='0'>"; break;
			case "21": $imgs = "<img src='../images/status24.png' border='0'>"; break;
 			case "23": $imgs = "<img src='../images/status44.png' border='0'>"; break;
			case "24": $imgs = "<img src='../images/status10.png' border='0'>"; break;
			case "25": $imgs = "<img src='../images/status26.png' border='0'>"; break;

			//case "31": $imgs = "<img src='../images/status26.png' border='0'>"; break;
			//case "32": $imgs = "<img src='../images/status26.png' border='0'>"; break;
			//case "33": $imgs = "<img src='../images/status26.png' border='0'>"; break;
			//case "34": $imgs = "<img src='../images/status26.png' border='0'>"; break;
			case "31": if($review_f_count != 0){$imgs = "<img src='../images/status26.png' border='0'>";}else{$imgs = "<img src='../images/status46.png' border='0'>";} break;  //검토대기 3차
			case "32": $imgs = "<img src='../images/status48.png' border='0'>"; break;  //검토대기 3차
			case "33": $imgs = "<img src='../images/status50.png' border='0'>"; break;  //검토대기 3차
			case "34": $imgs = "<img src='../images/status46.png' border='0'>"; break;  //검토대기 3차

			case "49": $imgs = "<span class='btn btn-danger'>부적합</span>"; break;
			case "50": $imgs = "<img src='../images/status27.png' border='0'>"; break;
			//case "51": $imgs = "<img src='../images/status28.png' border='0'>"; break;
			case "51": if($total[0]['result']=="1"){$imgs = "<img src='../images/status39.png' border='0'>"; break;}else{$imgs = "<img src='../images/status40.png' border='0'>"; break;}
		...
		switch($data['step']){
			case "1": $imgs = ""; break;
			case "2": $imgs = ""; break;
			case "3": $imgs = "<img src='../images/status29.png' border='0'>"; break;
			case "4": $imgs = "<img src='../images/status29.png' border='0'>"; break;
			case "5": $imgs = "<img src='../images/status30.png' border='0'>"; break;

			case "10": $imgs = "<img src='../images/status30.png' border='0'>"; break;
			case "11": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status30.png' border='0'>";} break;
			case "13": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status30.png' border='0'>";} break;
			case "14": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status31.png' border='0'>";}else{ $imgs = "<img src='../images/status31.png' border='0'>";} break;
			case "15": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status32.png' border='0'>";} break;

			case "20": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status32.png' border='0'>";} break;
			case "21": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status32.png' border='0'>";} break;
			case "23": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status32.png' border='0'>";} break;
			case "24": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status33.png' border='0'>";} break;
			case "25": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status34.png' border='0'>";} break;

			case "31": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status34.png' border='0'>";} break;
			case "32": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status34.png' border='0'>";} break;
			case "33": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status34.png' border='0'>";} break;
			case "34": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "<img src='../images/status35.png' border='0'>";}else{ $imgs = "<img src='../images/status34.png' border='0'>";} break;

			case "49": $imgs = "<span class='btn btn-danger'>부적합</span>"; break;
			case "50": $imgs = "<img src='../images/status35.png' border='0'>"; break;
			case "51": $imgs = "<img src='../images/status35.png' border='0'>"; break;
		...
		//echo $member['mb_id'];
		if($data['step']=="4"){
			$sql = "select * from ad_paper_review where parent_seq = '{$seq}' and mb_id = '{$member['mb_id']}'";
			$data	= sql_fetch($sql);
			if($data['rseq']){
				$imgs = "<img src='../images/status17.png' border='0'>";
			}
		}

		if($data['step']=="14"){
			$cnt = 0;
			$sql	= "select * from ad_paper_review where parent_seq = '{$seq}' and mb_id = '{$member['mb_id']}' order by rseq asc";
			$res	= sql_query($sql);
			while ($row = sql_fetch_array($res)){
				$cnt++;
			}
		...
		//if($data['step'] > 3){ // 2차 심사일경우 제외
		if($data['step'] > 3 && $data['step']!=14){
			$sql	= "select * from ad_paper_review where parent_seq = '{$seq}' and mb_id = '{$member['mb_id']}' order by rseq asc";
			$result = 0;
			$res	= sql_query($sql);
			while ($row = sql_fetch_array($res)){
			...
			if($result){
				$imgs = "<img src='../images/status35.png' border='0'>";
			}
		}
		...
		switch($data['step']){
			case "1": $imgs = "<img src='../images/status01.png' border='0'>"; break;
			case "2": $imgs = "<img src='../images/status02.png' border='0'>"; break;
			case "3": $imgs = "<img src='../images/status03.png' border='0'>"; break;
			case "4": $imgs = "<img src='../images/status04.png' border='0'>"; break;
			case "5": $imgs = "<img src='../images/status04.png' border='0'>"; break;

			case "10": $imgs = "<img src='../images/status05.png' border='0'>"; break;
			case "11": $imgs = "<img src='../images/status06.png' border='0'>"; break;
			case "13": $imgs = "<img src='../images/status36.png' border='0'>"; break;
			case "14": $imgs = "<img src='../images/status07.png' border='0'>"; break;
			case "15": $imgs = "<img src='../images/status07.png' border='0'>"; break;

			case "20": $imgs = "<img src='../images/status08.png' border='0'>"; break;
			case "21": $imgs = "<img src='../images/status09.png' border='0'>"; break;
			case "23": $imgs = "<img src='../images/status37.png' border='0'>"; break;
			case "24": $imgs = "<img src='../images/status10.png' border='0'>"; break;
			case "25": $imgs = "<img src='../images/status10.png' border='0'>"; break;

			//case "31": $imgs = "<img src='../images/status10.png' border='0'>"; break;
			//case "32": $imgs = "<img src='../images/status10.png' border='0'>"; break;
			//case "33": $imgs = "<img src='../images/status10.png' border='0'>"; break;
			//case "34": $imgs = "<img src='../images/status10.png' border='0'>"; break;

			case "31": if($review_f_count != 0){$imgs = "<img src='../images/status10.png' border='0'>";}else{$imgs = "<img src='../images/status47.png' border='0'>";} break;
			case "32": $imgs = "<img src='../images/status48.png' border='0'>"; break;
			case "33": $imgs = "<img src='../images/status49.png' border='0'>"; break;
			case "34": $imgs = "<img src='../images/status47.png' border='0'>"; break;

			case "49": $imgs = "<span class='btn btn-danger'>부적합</span>"; break;
			case "50": $imgs = "<img src='../images/status47.png' border='0'>"; break;
			case "51": $imgs = "<img src='../images/status11.png' border='0'>"; break;
		...
		if($data['step']=="51"){
			if($total[0]['result']=="1"){
				$imgs = "<img src='../images/status12.png' border='0'>";
			}else{
				$imgs = "<img src='../images/status13.png' border='0'>";
			}
		}
	}
	//$imgs = $member['mb_level'];
	...
	/*
	$imgs = "<img src=\"../images/state_04.png\" alt=\"논문수정\"/>";
	$imgs = "<img src=\"../images/state_05.png\" alt=\"투고완료\"/>";
	$imgs = "<img src=\"../images/state_06.png\" alt=\"편집위원장 결재대기\"/>";
	$imgs = "<img src=\"../images/state_07.png\" alt=\"편집위원장 결재완료\"/>";
	$imgs = "<img src=\"../images/state_08.png\" alt=\"총평대기\"/>";
	$imgs = "<img src=\"../images/state_09.png\" alt=\"총평완료\"/>";
	$imgs = "<img src=\"../images/state_10.png\" alt=\"게재승인\"/>";
	*/
	return $imgs;
}

function get_status_excel($seq, $type=null){
	global $g4, $member;
	$sql = "select * from ad_paper where seq = '{$seq}'";
	$data	= sql_fetch($sql);
	...
	if($member['mb_level'] >= 6){
		switch($data['step']){
			case "1": $imgs = "논문접수 대기"; break;
			case "2": $imgs = "심사위원 추천대기"; break;
			case "3": $imgs = "심사위원 선정대기"; break;
			case "4": $imgs = "심사중(1차)"; break;
			case "5": $imgs = "검토대기(1차)"; break;

			case "10": $imgs = "수정 요청(2차)"; break;
			case "11": $imgs = "논문접수 대기(2차)"; break;
			case "13": $imgs = "심사위원 선정대기(2차)"; break;
			case "14": $imgs = "심사중(2차)"; break;
			case "15": $imgs = "검토대기(2차)"; break;

			case "20": $imgs = "수정 요청(3차)"; break;
			case "21": $imgs = "논문접수 대기(3차)"; break;
			case "23": $imgs = "심사위원 선정대기(3차)"; break;
			case "24": $imgs = "심사중(3차)"; break;
			case "25": $imgs = "검토대기(3차)"; break;

			case "31": $imgs = "검토대기(3차)"; break;
			case "32": $imgs = "검토대기(3차)"; break;
			case "33": $imgs = "검토대기(3차)"; break;
			case "34": $imgs = "검토대기(3차)"; break;

			case "49": $imgs = "부적합"; break;
			case "50": $imgs = "최종심사 대기"; break;
			case "51": $imgs = "최종완료"; break;
		...
		switch($data['step']){
			...
			case "2": $imgs = ""; break;
			case "3": $imgs = ""; break;
			case "4": $imgs = "심사의견 대기(1차)"; break;
			case "5": $imgs = "심사의견 완료(1차)"; break;

			case "10": $imgs = "심사의견 완료(1차)"; break;
			case "11": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "심사 완료";}else{ $imgs = "심사의견 완료(1차)";} break;
			case "13": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "심사 완료";}else{ $imgs = "심사의견 완료(1차)";} break;
			case "14": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "심사 완료";}else{ $imgs = "심사의견 대기(2차)";} break;
			case "15": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "심사 완료";}else{ $imgs = "심사의견 완료(2차)";} break;

			case "20": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "심사 완료";}else{ $imgs = "심사의견 완료(2차)";} break;
			case "21": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "심사 완료";}else{ $imgs = "심사의견 완료(2차)";} break;
			case "23": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "심사 완료";}else{ $imgs = "심사의견 완료(2차)";} break;
			case "24": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "심사 완료";}else{ $imgs = "심사의견 대기(3차)";} break;
			case "25": if ($review[0]['result'] == 1 || $review[0]['result'] == 2 || $review[0]['result'] == 4){$imgs = "심사 완료";}else{ $imgs = "심사의견 완료(3차)";} break;

			case "49": $imgs = "부적합"; break;
			case "50": $imgs = "심사 완료"; break;
			case "51": $imgs = "심사 완료"; break;
		...
		switch($data['step']){
			case "1": $imgs = "논문투고"; break;
			case "2": $imgs = "접수완료"; break;
			case "3": $imgs = "심사대기(1차)"; break;
			case "4": $imgs = "심사 중(1차)"; break;
			case "5": $imgs = "심사 중(1차)"; break;

			case "10": $imgs = "수정요청(2차)"; break;
			case "11": $imgs = "수정완료(2차)"; break;
			case "13": $imgs = "접수완료(2차)"; break;
			case "14": $imgs = "심사 중(2차)"; break;
			case "15": $imgs = "심사 중(2차)"; break;

			case "20": $imgs = "수정요청(3차)"; break;
			case "21": $imgs = "수정완료(3차)"; break;
			case "23": $imgs = "접수완료(3차)"; break;
			case "24": $imgs = "심사 중(3차)"; break;
			case "25": $imgs = "심사 중(3차)"; break;

			case "31": $imgs = "심사 중(3차)"; break;
			case "32": $imgs = "심사 중(3차)"; break;
			case "33": $imgs = "심사 중(3차)"; break;
			case "34": $imgs = "심사 중(3차)"; break;

			case "49": $imgs = "부적합"; break;
			case "50": $imgs = "심사 중(3차)"; break;
			case "51": $imgs = "총평완료"; break;
...
function get_review_confirm($usr,$seq){
	$sql	= "select * from ad_reviewer_log where parent_seq = '{$seq}' and review_user = '{$usr}'";
	$mem	= sql_fetch($sql);
	if($mem['confirm'] == 'Y'){
		return "심사 승인";
	}else if($mem['confirm'] == 'N'){
		return "심사 거부";
	}else{
		if($usr == ''){
			return "";
...
function get_confirm_Y($mb_id){
	$sql = "select count(*) as num from ad_reviewer_log where review_user='{$mb_id}' and confirm='Y'";;
	$review	= sql_fetch($sql);
	return $review['num'];
}

function get_confirm_N($mb_id){
	$sql = "select count(*) as num from ad_reviewer_log where review_user='{$mb_id}' and confirm='N'";;
	$review	= sql_fetch($sql);
	return $review['num'];
}
...
function get_review_status($user,$seq,$step){
	...
	$sql = "select * from ad_paper_review where parent_seq = '{$seq}' and mb_id = '{$user}' and rstep = '{$rstep}'";
	$mem	= sql_fetch($sql);
	if($mem['result'] == '1'){
		return "/게재가";
	}else if($mem['result'] == '2'){
		return "/수정 후 게재가";
	}else if($mem['result'] == '3'){
		return "/수정 후 재심사";
	}else if($mem['result'] == '4'){
		return "/게재불가";
	}
}

function get_category_mail($num=null){
	$arr[] = array("cvalue"=>1, "ctext"=>"geochemistry@naver.com");
	$arr[] = array("cvalue"=>2, "ctext"=>"geophysical@naver.com");
	$arr[] = array("cvalue"=>3, "ctext"=>"rockmechanics@naver.com");
	$arr[] = array("cvalue"=>4, "ctext"=>"mineral@naver.com");
	$arr[] = array("cvalue"=>5, "ctext"=>"renewable@naver.com");
	$arr[] = array("cvalue"=>6, "ctext"=>"resource@naver.com");
	$arr[] = array("cvalue"=>7, "ctext"=>"petroleum@naver.com");
	$arr[] = array("cvalue"=>8, "ctext"=>"engineering@naver.com");
...
function get_member_level($num=null){
	//$arr[] = array("cvalue"=>1, "ctext"=>"");
	$arr[] = array("cvalue"=>2, "ctext"=>"정회원");
	//$arr[] = array("cvalue"=>3, "ctext"=>"");
	$arr[] = array("cvalue"=>4, "ctext"=>"심사위원");
	//$arr[] = array("cvalue"=>5, "ctext"=>"편집위원");
	//$arr[] = array("cvalue"=>6, "ctext"=>"시스템관리자");
	//$arr[] = array("cvalue"=>7, "ctext"=>"학생회원");
	//$arr[] = array("cvalue"=>8, "ctext"=>"대표간사");
	//$arr[] = array("cvalue"=>9, "ctext"=>"임시대표간사");
	$arr[] = array("cvalue"=>10, "ctext"=>"편집장");
