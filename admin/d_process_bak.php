...
if($_POST['mode']=="d_sub_reg"){
	...
	if($_FILES['modify_file'][tmp_name]){
		$tmp_file  = $_FILES['modify_file'][tmp_name];
		$filesize  = $_FILES['modify_file'][size];
		$filename  = $_FILES['modify_file'][name];
		...
		if (is_uploaded_file($tmp_file)){
			$error_code = move_uploaded_file($tmp_file, $dest_file) or die($_FILES['modify_file'][error]);
		}
		$file_sql = " , modify_file	= '/data/ddata/{$sfilename}' ";
	} else {
	...
	if($_FILES['response_data'][tmp_name]){
		...
		$file_sql .= " , response_data	= '/data/adata/{$sfilename5}' ";
	}
	$step = 3;
	if($_POST['step'] == 11){
		//$step = 14;
	...
	if($_POST['step'] == 1) $num_sql = " , number = '{$seq_number}' ";
	$numbers= str_pad($seq_number,3,'0',STR_PAD_LEFT);
	### =================================================================
	$modify_cnt = 0;
	$total_result = 0;
	$i = 0;
	...
	if($tcount > 0){
		...
		// 최종편집장 심사 경우외에는 무조건 재심사이므로 단계 분기 제거 hjshyo 170825
		if($fr_result2['result'] == '3' || ($_POST['step']==11 && $review_3_count < 1)){
			## 수정 후 재심사는 편집위원장에게 메일 발송하고 편집위원장 심사 단계 진행. step 34
			$usql = "UPDATE ad_paper SET settle_date = now(), step = 34 {$file_sql} WHERE seq = '{$_POST['seq']}'";
			##############
			$body = "
			{$mail_header}
			...
			##############
		}else if($fr_result2['result'] == '2'){
			## 수정 후 게재가는 메일 발송 없이 최종 결과 등록으로 이동. step 50
			$usql = "UPDATE ad_paper SET settle_date = now(), step = 50 {$file_sql} WHERE seq = '{$_POST['seq']}'";
			##############
			$body = "
			{$mail_header}
		...
		$number = str_pad($_POST['number'],3,'0',STR_PAD_LEFT);
		$usql = "UPDATE ad_paper SET review_category_target	 = '{$_POST['review_category_target']}', manuscript = '{$_POST['manuscript']}', submit_date = now(), step = {$step} {$num_sql} {$file_sql} WHERE seq = '{$_POST['seq']}'";
		sql_query($usql);
...
HTML;
			...
			$body = "
			{$mail_header}
			<p>{$info['institute_title']} [{$info['journal_title']}] 심사위원 추천의뢰 {$info['abbr']}-{$id_year}-{$numbers}</p>
			<p>편집위원장님께  {$review_category_mailaddress}</p>
			<p>안녕하세요?</p>
			...
			";
			##############
			$mail->sendInput($main_editor, '편집장', $body, "[{$info['journal_title']}] 심사위원 추천의뢰 {$info['abbr']}-{$id_year}-{$numbers}");
			$mail->sendInput($_POST['mb_id'], $_POST['mb_name'], $body2, "[{$info['journal_title']}] 논문접수 등록완료({$_POST['mb_name']} 귀하) {$info['abbr']}-{$id_year}-{$numbers}");
			##############
		}
		### =================================================================
	}
	...
	// 확정시 메일 발송으로 임시로 제거 160414
	/* foreach(array('a','b','c') as $v) {
		if($data['review_'.$v.'_conf']=='Y'){
			$cnt++;
			$number = str_pad($_POST['number'],3,'0',STR_PAD_LEFT);
			if($_POST['review_'.$v.'_date']) ${'review_'.$v.'_date'} = date("Y년 m월 d일", strtotime($_POST['review_'.$v.'_date']));
			else {
				${'review_'.$v.'_date'} = date("Y년 m월 d일", strtotime('+2 week'));
				$_POST['review_'.$v.'_date'] = date("Ymd", strtotime('+2 week'));
			}
			if($_POST['step'] == 13 && $_POST['review_'.$v.'_result'] == "3"){
				##############
				$body = "
				{$mail_header}
				<p>{$info['institute_title']} [{$info['journal_title']}] 2차 논문 심사 의뢰 {$info['abbr']}-{$id_year}-{$number}</p>
				<p>{$_POST['review_'.$v.'_name']} 심사위원님께</p>
				<p>안녕하세요?</p>
				<p>&nbsp;아래와 같이 1차 심사한 논문의 저자수정이 완료되어 2차 심사를 요청 드립니다. 심사기간은 {${'review_'.$v.'_date'}} 이내이며, 이 작업을 수행하시려면 아래 링크를 클릭하시거나, 온라인투고시스템에 등록하신 후 로그인 하여 작업을 진행하시기 바랍니다.</p>
				</td></tr>
				<tr><td height='15'></td></tr>
				<tr>
				<td height='51' align='center' valign='middle'><a href='http://{$_SERVER['HTTP_HOST']}/admin/b_sub01_write.php?seq={$_POST['seq']}' target='_blank'>2차 논문심사</a></td>
				</tr>
				<tr><td height='15'></td></tr>
				<tr><td height='20' align='left' valign='top'>
				<p>{$info['institute_title']}지가 수준 높은 학술지가 될 수 있도록 당신의 참여에 대해 감사드립니다.</p>
				</td></tr>
				<tr><td height='51' align='left' valign='top'>
				<p>원고 세부 사항</p>
				<p>제목 : {$_POST['title']}</p>
				...
				{$mail_footer}
				";
				$mail->sendInput($_POST['review_'.$v.'_user'], $_POST['review_'.$v.'_name'], $body, "[{$info['journal_title']}] 2차 논문 심사 의뢰({$_POST['review_'.$v.'_name']} 귀하)");
			}
			if($_POST['step'] == 3){
				##############
				$body = "
				{$mail_header}
				<p>{$info['institute_title']} [{$info['journal_title']}] 1차 논문 심사 의뢰 {$info['abbr']}-{$id_year}-{$number}</p>
				<p>{$_POST['review_'.$v.'_name']} 심사위원님께</p>
				<p>안녕하세요?</p>
				<p>&nbsp; {$info['journal_title']} 심사를 요청드립니다. 심사기간은 {${'review_'.$v.'_date'}} 이내이며, 이 작업을 수행하시려면 아래 링크를 클릭하시거나, 온라인투고시스템에 등록하신 후 로그인 하여 작업을 진행하시기 바랍니다.</p>
				</td></tr>
				<tr><td height='15'></td></tr>
				<tr>
				<td height='51' align='center' valign='middle'><a href='http://{$_SERVER['HTTP_HOST']}/admin/b_sub01_write.php?seq={$_POST['seq']}' target='_blank'>1차 논문심사</a></td>
				</tr>
				<tr><td height='15'></td></tr>
				<tr><td height='20' align='left' valign='top'>
				<p>바쁘신 가운데, 심사를 수락해주셔서 진심으로 감사드립니다.</p>
				</td></tr>
				<tr><td height='51' align='left' valign='top'>
				<p>원고 세부 사항</p>
				<p>제목 : {$_POST['title']}</p>
				...
				{$mail_footer}
				";
				$mail->sendInput($_POST['review_'.$v.'_user'], $_POST['review_'.$v.'_name'], $body, "[{$info['journal_title']}] 1차 논문 심사 의뢰({$_POST['review_'.$v.'_name']} 귀하)");
				##############
			}
		}
	} */
...
";
	...
	$number = str_pad($_POST['number'],3,'0',STR_PAD_LEFT);
	// 편집위원장 메일 주소 가지고 오기. 편집위원장의 카테고리 분야에 따라 메일 주소를 가지고 와서 발송 함.
	//$review_category_mailaddress = get_category_mail($_POST['review_category']);
	$review_category_mailaddress = $main_editor;
	### REVIEW CHECK
	if($_POST['step']==15){
		$sql = "select * from ad_paper_review where parent_seq = '{$_POST['seq']}' and rstep = 2";
		$step = 31;
	}else{
		$sql = "select * from ad_paper_review where parent_seq = '{$_POST['seq']}' and rstep = 1";
		$step = 10;
...
<p> 
먼저 「{$info['journal_title']}」에 논문을 투고해주셔서 진심으로 감사드립니다.
</p>
<p> 
수합된 심사결과를 투고시스템(<a href="http://{$_SERVER['HTTP_HOST']}">http://{$_SERVER['HTTP_HOST']}</a>)에서 확인부탁드립니다.
</p>
<p>
감사합니다. 
</p>
<p>
- {$info['institute_title']} 편집위원회 드림 -
</p>
{$mail_footer}
HTML;
	}
	...
	while ($row = sql_fetch_array($res)){
			if($row['result']==3) $modify_cnt++;
			if($row['result']==4) $decline_cnt++;
			$result_sum = $result_sum + $row['result'];
	}
	// 게재불가가 1개에서 수정후 재심사가 1개라도 있으면 게재보류 처리
/* 	if(($decline_cnt==1 && $result_sum < 6){
		...
		<p>편집위원장님</p>
		<p>&nbsp; 본 학회 [{$info['journal_title']}]에 {$_POST['mb_name']}님이 투고한 원고의 심사결과가 편집위원장님에 의해 '게재보류'로 최종논문 검토 완료되었습니다. </p>
		</td></tr>
		<tr><td height='15'></td></tr>
		<tr><td height='51' align='left' valign='top'>
		<p>원고 세부 사항</p>
	...
	if($_POST['result'] == 4){
		...
		// 1차 수정요청 메일 발송.
		$sql = "UPDATE ad_paper SET settle_date = now(), step = '{$step}', edit_comment='{$_POST['edit_comment']}' WHERE seq = '{$_POST['seq']}'";
		$mail->sendInput($_POST['mb_id'], $_POST['mb_name'], $body_toauth, $subject);
		$mail->sendInput($main_editor, '편집장', $body, $subject);
		##############
	}else{
		//$sql = "UPDATE ad_paper SET settle_date = now(), step = 50 WHERE seq = '{$_POST['seq']}'";
		$sql = "UPDATE ad_paper SET settle_date = now(), step = 31, edit_comment='{$_POST['edit_comment']}' WHERE seq = '{$_POST['seq']}'";
		// 편집위원장 메일 주소 가지고 오기. 편집위원장의 카테고리 분야에 따라 메일 주소를 가지고 와서 발송 함.
		$body = "
		{$mail_header}
		<p>{$info['institute_title']} [{$info['journal_title']}] 최종논문 검토 요청 {$info['abbr']}-{$id_year}-{$number}</p>
		<p>편집위원장님께 {$review_category_mailaddress}</p>
		<p>안녕하세요?</p>
		<p>아래의 원고가 최종 완료되었기에 논문 확인요청을 드립니다. 최종원고를 보시고 심사 의견대로 수정이 잘 이루어졌는지, 혹은 수정이 미흡하여 추가로 수정해야 하는지 확인 부탁드립니다. 논문에 대한 자세한 내용을 확인 하시려면 아래 링크를 클릭하신 후 진행하시거나, 온라인투고시스템에 편집위원장 아이디로 로그인 하신 후 이용하시기 바랍니다.</p>
		</td></tr>
		<tr><td height='15'></td></tr>
		<tr>
		<td height='51' align='center' valign='middle'><a href='http://{$_SERVER['HTTP_HOST']}/admin/d_sub09_write.php?seq={$_POST['seq']}' target='_blank'>투고논문 확인</a></td>
...
### 논문 총평등록
	...
	if($_POST['result'] == "1"){
		...
		//$mail->sendInput($_POST['mb_id'], $_POST['mb_name'], $body, "[{$info['journal_title']}] 논문 최종결과 등록 완료({$_POST['mb_name']} 귀하)");
		##############
		...
		if($total_count > 0){
			...
			##############
			$mail->Subject    = "{$info['institute_title']}지 논문 심사결과 양해말씀 {$info['abbr']}-{$id_year}-{$number}";
			$body = "<html xmlns='http://www.w3.org/1999/xhtml'><head><meta http-equiv='Content-Type' content='text/html; charset=utf-8'/><title></title></head>
			<body>
			<table width='750' border='0' cellspacing='0' cellpadding='0'><tr><td height='85' align='center' valign='top'><img src='http://{$_SERVER['HTTP_HOST']}/images/mail_title.png' width='750' height='65' /></td></tr><tr><td height='15'></td></tr>
			<tr><td height='50' align='left' valign='top'>
			<p>{$info['institute_title']}지 논문 심사결과 양해말씀 {$info['abbr']}-{$id_year}-{$number}</p>
			<p>심사위원님께</p>
			<p>안녕하세요?</p>
			<p>&nbsp; 안녕하세요?
			심사위원님께서 게재불가를 선택하신 논문이 다른 심사위원과 편집위원장의 결정에 의해 게재가 확정 되어 이에 양해의 메일을 보내드립니다.
			</p>
			</td></tr>
			<tr><td height='15'></td></tr>
			<tr><td height='51' align='left' valign='top'>
			...
			<td width='240' height='80'><img src='http://{$_SERVER['HTTP_HOST']}/images/mail_logo.png' width='240' height='80' /></td>
			<td width='10'></td><td align='left'>
			<p>{$info['institute_title']} <br />
			경기도 안양시 만안구 삼막로 155, 경인교육대학교 미술교육과<br />
			...
			$mail->MsgHTML($body);
			//$address = $mem['mb_id'];
			//$mail->AddAddress($address, $_POST['mb_name']);
			$address = $main_editor;
			$mail->AddAddress($address, "{$info['institute_title']}");
			if(!$mail->Send()) {
		...
		*/
		...
		if($total_count > 0){
			...
			<p>안녕하세요?</p>
			<p>&nbsp; 안녕하세요?
			심사위원님께서 게재승인을 선택하신 논문이 다른 심사위원과 편집위원장의 결정에 의해 게재가 거부 되어 이에 양해의 메일을 보내드립니다.
			</p>
			</td></tr>
			<tr><td height='15'></td></tr>
			<tr><td height='51' align='left' valign='top'>
...
### 심사윈원
	...
	if($_POST['mb_no']){
		...
		//$category_str = implode("|", $_POST['field']);
...
/* 	$nm = $_GET['nm'];
	...
	$number = str_pad($row['number'],3,'0',STR_PAD_LEFT);
	// 기본 최종심사날 세팅
