...
###
$write_pages = get_paging(10, $page, $total_page, "./a_sub02.php?page=");
?>
...
<td valign="top">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td background="../images/titlebg.png"><img src="../images/01_title02.png" /></td>
	</tr>
	<tr>
		...
		<tr>
			...
			<!--tr>
				...
				<th rowspan="2"><strong>투고자</strong><br />Author</th>
				<th colspan="5"><strong>논문 접수</strong></th>
			...
			<tr>
				...
				<th><strong>게재확정일</strong><br />Accepted Date</th>
			</tr-->
			<tr>
				<th rowspan="2"><strong>No</strong></th>
			...
			<tr>
				<td><?=$list[$i]['num']?></td>
				<td>
					<?=get_status($list[$i]['seq'], 'writer')?><br/>
					<?
					$sql = "select * from ad_paper_total where parent_seq = '{$list[$i]['seq']}' order by regdate desc";
					$data	= sql_fetch($sql);
					?>
					<? if($data['rfile']){ ?>
					<a href="/down.php?link=<?=$data['rfile']?>"><img src="../images/status38.png"  border="0" align="absmiddle" style="padding-top:5px;"/></a>
					<? } ?>
				</td>
				<?
				$cyear = date("y");
				...
				<td><a href="./a_sub02_write.php?seq=<?=$list[$i]['seq']?>"><font color="#0088de" style="font-weight:bold"><?=$list[$i]['title']?></a></td>
